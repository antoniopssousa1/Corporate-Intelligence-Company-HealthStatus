...
def load_companies():
    ...
    conn = get_connection()
    df = pd.read_sql("SELECT ticker, company_name, sector FROM silver_companies ORDER BY ticker", conn)
    return df
...
def create_ratio_bar_chart(ratios_df, title):
    """Create horizontal bar chart for ratios"""
    fig = go.Figure()
    
    # Define benchmark ranges for each ratio
    benchmarks = {
        'current_ratio': {'good': 1.5, 'name': 'Current Ratio'},
        'quick_ratio': {'good': 1.0, 'name': 'Quick Ratio'},
        'cash_ratio': {'good': 0.25, 'name': 'Cash Ratio'},
        'gross_margin': {'good': 0.3, 'name': 'Gross Margin'},
        'operating_margin': {'good': 0.15, 'name': 'Operating Margin'},
        'net_margin': {'good': 0.1, 'name': 'Net Margin'},
        'roe': {'good': 0.15, 'name': 'ROE'},
        'roa': {'good': 0.05, 'name': 'ROA'},
        'debt_to_equity': {'good': 1.0, 'name': 'Debt/Equity', 'inverse': True},
        'debt_to_assets': {'good': 0.5, 'name': 'Debt/Assets', 'inverse': True}
    }
    
    for col in ratios_df.columns:
        if col in benchmarks and ratios_df[col].iloc[0] is not None:
            val = ratios_df[col].iloc[0]
            benchmark = benchmarks[col]
            
            # Determine color (inverse for debt ratios - lower is better)
            if benchmark.get('inverse', False):
                color = '#38ef7d' if val <= benchmark['good'] else '#eb5757'
            else:
                color = '#38ef7d' if val >= benchmark['good'] else '#eb5757'
...
def main():
    ...
    try:
        ...
        st.info("Make sure PostgreSQL is running and the pipeline has been executed.")
        return
    ...
    with col1:
        ...
        # Analysis notes
        if latest_health['analysis_notes']:
            st.markdown("### 📝 Key Insights")
            notes = latest_health['analysis_notes'].split('; ')
            for note in notes:
                st.markdown(f"- {note}")
    ...
    with col3:
        ...
        colors = [get_health_color(s) for s in health_sorted['health_status']]
    ...
    <div style="text-align: center; color: #666;">
        <p>📊 Financial Health Dashboard | Data from Gold Layer | Built with Streamlit</p>
